...
/**
 * The SmsProviders class defines a message of the messaging system defined
 * by the GreenballMessages grammar. It provides methods for packing a SmsProviders object
 * into a byte array or string, and for unpacking a byte array or string
 * back into a SmsProviders object.
 */
public class SmsProviders {
   private static final int ID = GreenballMessagesEnum.SMS_PROVIDERS.getId();

   /**
    * The number of bits required to represent the SmsProvidersEnum IDs.
    * # of attributes: 2
    * # of PEnumRefs: 0
    */
   ...
   private static Logger log4j = Logger.getLogger(SmsProviders.class);

   // Entered buildClassEnum()
   private enum SmsProvidersEnum {
      // This is a directory item. It's offset and length cannot be determined
      // until runtime.
      SMS_PROVIDER(0, -1, 0, true),
      ...
      SmsProvidersEnum(int id, int offsetInBits, int length, boolean isDirectoryEntry) {
         this.id = id;
    ...
    */
   private List<SmsProviderSubType> smsProvider;
   private String sessionId;

   private String theErrorMsg;

   /**
    * The SmsProviders no-arg constructor.
    */
   public SmsProviders() {}

   /**
    * The SmsProviders constructor.
    * See buildParameterAnnotations() in JBitByBitGenerator.java.
    *
    * @param smsProvider The smsProvider message attribute.
    * @param sessionId The sessionId message attribute.
    */
   public SmsProviders(List<SmsProviderSubType> smsProvider,
                       String sessionId) {
   ...
   /**
    * This method packs a SmsProviders object into a byte array. If a required attribute is not
    * provided an exception will be thrown.
    *
    * @return A byte array which represents a SmsProviders message object.
   ...
   public byte[] pack() throws MissingAttributeException {
      ...
      int byteArraySize = getSmsProvidersSize();
      ...
      SmsProvidersEnum.SMS_PROVIDER.setLength(this.getSmsProvider().size());
      SmsProvidersEnum.SESSION_ID.setLength(this.getSessionId() != null ? this.getSessionId().length() : 0);

      int directorySize = getSmsProvidersDirectorySize();
      if (directorySize != 0) {
         ...
         for (SmsProvidersEnum smsProvidersEnum : SmsProvidersEnum.values()) {
            if (smsProvidersEnum.isDirectoryEntry == true) {
               int id = smsProvidersEnum.getId();
               int length = smsProvidersEnum.getLength();
               ...
               // Insert the data
               if (id == SmsProvidersEnum.SMS_PROVIDER.getId()) {
                  log4j.debug(String.format("inserting %s at offset %d", "smsProvider", offset));
                  for (SmsProviderSubType smsProviderSubType : this.smsProvider) {
                     byte[] bytes = smsProviderSubType.pack();
               ...
               else if (id == SmsProvidersEnum.SESSION_ID.getId()) {
   ...
   /**
    * This method unpacks a byte array containing a SmsProviders object.
    *
    * @return The byte array as a SmsProviders message object.
    */
   public SmsProviders unpack(byte[] ba) {
      ...
      for (GreenballMessagesDirectoryEntry directoryEntry : theDirectory) {
         ...
         if (id == SmsProvidersEnum.SESSION_ID.getId()) {
            smsProviders.setSessionId(GreenballMessagesUtility.getString(ba, offset, length));
         }
         else if (id == SmsProvidersEnum.SMS_PROVIDER.getId()) {
            byte[] bytes = new byte[length];
            for (int index = 0; index < length; index++) {
               bytes[index] = GreenballMessagesUtility.getByte(ba, (offset + index) * 8);
            }
            smsProviders.getSmsProvider().add(new SmsProviderSubType().unpack(bytes));
         }
      }
   ...
   /**
    * This method packs a SmsProviders object into a String. If a required attribute is not
    * provided an exception will be thrown.
    *
    * @return A String which represents a SmsProviders message object.
    * @throws MissingAttributeException
    */
   public String packStr() throws MissingAttributeException {
       ...
       */
      InitializeSmsProviders initializeSmsProviders = new InitializeSmsProviders(this);
      Element element = initializeSmsProviders.initialize();
    ...
    */
   public SmsProviders unpack(String buffer) {
      final String METHOD = "unpack()";
      log4j.debug("Entered " + METHOD);
      ...
      /*
       * Instantiate the SmsProviders object.
       */
       InstantiateSmsProviders instantiateSmsProviders = new InstantiateSmsProviders(element);
       SmsProviders smsProviders = instantiateSmsProviders.instantiate();

      /*
       * Verify that a value was provided for all mandatory attributes.
       */
      String result = instantiateSmsProviders.validate();
      if (result != null) {
   ...
   /**
    * This method sets the smsProvider attribute. Setting of this value is required.
    * @param smsProvider The smsProvider.
    */
   public void setSmsProvider(List<SmsProviderSubType> smsProvider) {
      this.smsProvider = smsProvider;
   }

   /**
    * This method gets the smsProvider attribute. The smsProvider attribute is a required attribute.
    */
   public List<SmsProviderSubType> getSmsProvider() {
      if (this.smsProvider == null) {
         this.smsProvider = new ArrayList<SmsProviderSubType>();
      }
    ...
    */
   private int getSmsProvidersDirectorySize() {
      int directorySize = 0;

      // smsProvider
      directorySize += getSmsProvider().size();
      // sessionId
      directorySize++;
   ...
   /**
    * This method gets the size of the SmsProviders object.
    * @return The size in bytes.
    */
   public int getSmsProvidersSize() {
      int size = 0;
      ...
      // Add size of smsProvider
      for (SmsProviderSubType smsProviderSubType: getSmsProvider()) {
         size += smsProviderSubType.getSmsProviderSubTypeSize();
      }
   ...
   public String toString() {
      ...
      if (smsProvider != null) {
         for (SmsProviderSubType smsProviderSmsProviderSubType : smsProvider) {
            result.append(smsProviderSmsProviderSubType.toString());
         }
      }
   ...
   public boolean equals(Object obj) {
      ...
      SmsProviders smsProviders = (SmsProviders) obj;
      if (!smsProviders.getSessionId().equals(this.getSessionId())) {
         return false;
      }
