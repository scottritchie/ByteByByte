/*
 * WARNING: This file was generated by BitByBitGenerator.xtend (see
 * generateSubTypeInstantiationFile()).
 * Do not edit.
 */
package com.viacron.greenball.app.bbb;
import java.util.ArrayList;
import java.util.List;

import org.apache.log4j.Logger;

import com.viacron.utils.StringMessageBuffer.parser.AbstractAttribute;
import com.viacron.utils.StringMessageBuffer.parser.Element;
import com.viacron.utils.StringMessageBuffer.parser.KeyValuePair;

public class InstantiateCrossReference {
private static Logger log4j = Logger.getLogger(InstantiateCrossReference.class);

private Element element;
private CrossReference crossReference;

/*
 * The isUpdated flags. If an attribute is required by the grammar then a value
 * must be provided, and the isUpdated flag will be set when the value is
 * obtained during unpacking. After the object is constructed during unpacking, 
 * validation will be performed to make sure a value was provided for all attributes
 * that are not optional. 
 * See buildFlags() in JBitByBitGenerator.java.
 */
private boolean isNameUpdated = false;
private boolean isReferencedObjectUpdated = false;
private boolean isMultiUpdated = false;

/**
 * This method constructs a InstantiateCrossReference object based on an Element
 * object.
 * 
 * @param element
 *            The Element.
 */
public InstantiateCrossReference(Element element) {
this.element = element;
this.crossReference = new CrossReference();
}

/**
 * The instantiate method instantiates a CrossReference object.
 * 
 * @return The CrossReference.
 */
protected CrossReference instantiate() {
final String METHOD = "instantiate()";
log4j.debug("Entered " + METHOD);

log4j.debug("element: " + element.getName());

for (AbstractAttribute abstractAttribute : element.getAbstractAttributes()) {
      // Entered instantiateSubTypeElements()
   // Leaving instantiateSubTypeElements()


   for (KeyValuePair kvp : abstractAttribute.getKeyValuePairs()) {
         // Entered instantiateSubTypeAttributes()
   if (kvp.getKey().equals("name")) {
    this.isNameUpdated = true;
   log4j.debug("setting value of \"name\" to: " + kvp.getValue());
   crossReference.setName(kvp.getValue());
   }
   else if (kvp.getKey().equals("referencedObject")) {
    this.isReferencedObjectUpdated = true;
   log4j.debug("setting value of \"referencedObject\" to: " + kvp.getValue());
   crossReference.setReferencedObject(kvp.getValue());
   }
   else if (kvp.getKey().equals("multi")) {
    this.isMultiUpdated = true;
   log4j.debug("setting value of \"multi\" to: " + kvp.getValue());
   crossReference.setMulti(kvp.getValue().equals("true"));
   }
   // Leaving instantiateSubTypeAttributes()

   }
}

log4j.debug("Leaving " + METHOD);
return crossReference;
}

/**
 * The validate method checks that a value was provided for all required 
 * attributes.
 *
 * @return The error message or null if no error occurred.
 */
protected String validate() {
final String METHOD = "validate()";
log4j.debug("Entered " + METHOD);

String result = null;

// Entered buildValidationMethod()
if (!this.isNameUpdated) {
   String msg = "Name is required, but no value was provided.";
   if (result == null) {
      result = msg;
   }
}

if (!this.isReferencedObjectUpdated) {
   String msg = "ReferencedObject is required, but no value was provided.";
   if (result == null) {
      result = msg;
   }
   else {
      result = result.concat("\n" + msg);
   }
}

if (!this.isMultiUpdated) {
   String msg = "Multi is required, but no value was provided.";
   if (result == null) {
      result = msg;
   }
   else {
      result = result.concat("\n" + msg);
   }
}


if (result != null) {
   log4j.debug("result: " + result);
}

log4j.debug("Leaving " + METHOD);
return result;
}
}
