...
public class InstantiateRequestGrammar {
   private static Logger log4j = Logger.getLogger(InstantiateRequestGrammar.class);
   ...
   /**
    * The InstantiateRequestGrammar class instantiates the RequestGrammar object.
    *
    * @param element The root element returned by the parser after parsing the string
    *                representing the RequestGrammar.
    */
   public InstantiateRequestGrammar(Element element) {
      this.element = element;   
      this.requestGrammar = new RequestGrammar();
   }

   /**
    * This method instantiates the RequestGrammar.
    *
    * @return The RequestGrammar.
    */
   public RequestGrammar instantiate() {
      final String METHOD = "instantiate()";
      log4j.debug("Entered " + METHOD);

      log4j.debug("RequestGrammar: " + element.getName());
      log4j.debug("# of attributes: " + element.getAbstractAttributes().size());

      instantiateRequestGrammarAttributes(element);
      ...
      instantiateRequestGrammarSubTypeRefs(e);
      }
      }
      ...
      /*
       * This method instantiates the RequestGrammar attributes.
       * 
       * @param Element element
       */
      private void instantiateRequestGrammarAttributes(Element element) {
      final String METHOD = "instantiateRequestGrammarAttributes()";
      log4j.debug("Entered " + METHOD);
      ...
      for (AbstractAttribute attr : element.getAbstractAttributes()) {
         for (KeyValuePair kvp : attr.getKeyValuePairs()) {
            ...
            if (kvp.getKey().equals("orgId")) {
               ...
               requestGrammar.setOrgId(kvp.getValue());
            }
            ...
            if (kvp.getKey().equals("orgName")) {
               ...
               requestGrammar.setOrgName(kvp.getValue());
            }
            // Leaving instantiateMsgAttribute()
            // Entered instantiateMsgAttribute()
            if (kvp.getKey().equals("userName")) {
               ...
               requestGrammar.setUserName(kvp.getValue());
            }
            // Leaving instantiateMsgAttribute()
            // Entered instantiateMsgAttribute()
            if (kvp.getKey().equals("template")) {
               log4j.debug("setting value of \"template\" to: " + kvp.getValue());
               requestGrammar.setTemplate(kvp.getValue());
            }
            // Leaving instantiateMsgAttribute()
            // Entered instantiateMsgAttribute()
            if (kvp.getKey().equals("sessionId")) {
               log4j.debug("setting value of \"sessionId\" to: " + kvp.getValue());
               requestGrammar.setSessionId(kvp.getValue());
            }
            // Leaving instantiateMsgAttribute()
         }
      }
   ...
   /*
    * This method instantiates the RequestGrammar SubType references.
    * 
    * @param Element element
    */
   private void instantiateRequestGrammarSubTypeRefs(Element element) {
      final String METHOD = "instantiateRequestGrammarSubTypeRefs()";
      log4j.debug("Entered " + METHOD);
