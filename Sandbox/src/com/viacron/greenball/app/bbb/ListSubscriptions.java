...
/**
 * The ListSubscriptions class defines a message of the messaging system defined
 * by the GreenballMessages grammar. It provides methods for packing a ListSubscriptions object
 * into a byte array or string, and for unpacking a byte array or string
 * back into a ListSubscriptions object.
 */
public class ListSubscriptions {
   private static final int ID = GreenballMessagesEnum.LIST_SUBSCRIPTIONS.getId();

   /**
    * The number of bits required to represent the ListSubscriptionsEnum IDs.
    * # of attributes: 2
    * # of PEnumRefs: 0
    */
   ...
   private static Logger log4j = Logger.getLogger(ListSubscriptions.class);

   // Entered buildClassEnum()
   private enum ListSubscriptionsEnum {
      // This is a directory item. It's offset and length cannot be determined
      // until runtime.
      ORGANIZATIONS(0, -1, 0, true),
      ...
      ListSubscriptionsEnum(int id, int offsetInBits, int length, boolean isDirectoryEntry) {
         this.id = id;
   ...
   /**
    * The ListSubscriptions no-arg constructor.
    */
   public ListSubscriptions() {}

   /**
    * The ListSubscriptions constructor.
    ...
    */
   public ListSubscriptions(List<OrganizationSubType> organizations,
                            String sessionId) {
   ...
   /**
    * This method packs a ListSubscriptions object into a byte array. If a required attribute is not
    * provided an exception will be thrown.
    *
    * @return A byte array which represents a ListSubscriptions message object.
   ...
   public byte[] pack() throws MissingAttributeException {
      ...
      int byteArraySize = getListSubscriptionsSize();
      ...
      ListSubscriptionsEnum.ORGANIZATIONS.setLength(this.getOrganizations().size());
      ListSubscriptionsEnum.SESSION_ID.setLength(this.getSessionId() != null ? this.getSessionId().length() : 0);

      int directorySize = getListSubscriptionsDirectorySize();
      if (directorySize != 0) {
         ...
         for (ListSubscriptionsEnum listSubscriptionsEnum : ListSubscriptionsEnum.values()) {
            if (listSubscriptionsEnum.isDirectoryEntry == true) {
               int id = listSubscriptionsEnum.getId();
               int length = listSubscriptionsEnum.getLength();
               ...
               // Insert the data
               if (id == ListSubscriptionsEnum.ORGANIZATIONS.getId()) {
               ...
               else if (id == ListSubscriptionsEnum.SESSION_ID.getId()) {
   ...
   /**
    * This method unpacks a byte array containing a ListSubscriptions object.
    *
    * @return The byte array as a ListSubscriptions message object.
    */
   public ListSubscriptions unpack(byte[] ba) {
      ...
      for (GreenballMessagesDirectoryEntry directoryEntry : theDirectory) {
         ...
         if (id == ListSubscriptionsEnum.SESSION_ID.getId()) {
            listSubscriptions.setSessionId(GreenballMessagesUtility.getString(ba, offset, length));
         }
         else if (id == ListSubscriptionsEnum.ORGANIZATIONS.getId()) {
            byte[] bytes = new byte[length];
            for (int index = 0; index < length; index++) {
               bytes[index] = GreenballMessagesUtility.getByte(ba, (offset + index) * 8);
            }
            listSubscriptions.getOrganizations().add(new OrganizationSubType().unpack(bytes));
         }
      }
   ...
   /**
    * This method packs a ListSubscriptions object into a String. If a required attribute is not
    * provided an exception will be thrown.
    *
    * @return A String which represents a ListSubscriptions message object.
    * @throws MissingAttributeException
    */
   public String packStr() throws MissingAttributeException {
       ...
       */
      InitializeListSubscriptions initializeListSubscriptions = new InitializeListSubscriptions(this);
      Element element = initializeListSubscriptions.initialize();
    ...
    */
   public ListSubscriptions unpack(String buffer) {
      final String METHOD = "unpack()";
      log4j.debug("Entered " + METHOD);
      ...
      /*
       * Instantiate the ListSubscriptions object.
       */
       InstantiateListSubscriptions instantiateListSubscriptions = new InstantiateListSubscriptions(element);
       ListSubscriptions listSubscriptions = instantiateListSubscriptions.instantiate();

      /*
       * Verify that a value was provided for all mandatory attributes.
       */
      String result = instantiateListSubscriptions.validate();
      if (result != null) {
    ...
    */
   private int getListSubscriptionsDirectorySize() {
      int directorySize = 0;
   ...
   /**
    * This method gets the size of the ListSubscriptions object.
    * @return The size in bytes.
    */
   public int getListSubscriptionsSize() {
      int size = 0;
   ...
   public boolean equals(Object obj) {
      ...
      if (!listSubscriptions.getSessionId().equals(this.getSessionId())) {
         return false;
      }
