...
/**
 * The SmsProviderRequest class defines a message of the messaging system defined
 * by the GreenballMessages grammar. It provides methods for packing a SmsProviderRequest object
 * into a byte array or string, and for unpacking a byte array or string
 * back into a SmsProviderRequest object.
 */
public class SmsProviderRequest {
   private static final int ID = GreenballMessagesEnum.SMS_PROVIDER_REQUEST.getId();

   /**
    * The number of bits required to represent the SmsProviderRequestEnum IDs.
    * # of attributes: 1
    * # of PEnumRefs: 0
    */
   ...
   private static Logger log4j = Logger.getLogger(SmsProviderRequest.class);

   // Entered buildClassEnum()
   private enum SmsProviderRequestEnum {
      ...
      SmsProviderRequestEnum(int id, int offsetInBits, int length, boolean isDirectoryEntry) {
         this.id = id;
   ...
   /**
    * This method packs a SmsProviderRequest object into a byte array. If a required attribute is not
    * provided an exception will be thrown.
    *
    * @return A byte array which represents a SmsProviderRequest message object.
   ...
   public byte[] pack() throws MissingAttributeException {
      ...
      int byteArraySize = getSmsProviderRequestSize();
      ...
      SmsProviderRequestEnum.SESSION_ID.setLength(this.getSessionId() != null ? this.getSessionId().length() : 0);

      int directorySize = getSmsProviderRequestDirectorySize();
      if (directorySize != 0) {
         ...
         for (SmsProviderRequestEnum smsProviderRequestEnum : SmsProviderRequestEnum.values()) {
            if (smsProviderRequestEnum.isDirectoryEntry == true) {
               int id = smsProviderRequestEnum.getId();
               int length = smsProviderRequestEnum.getLength();
               ...
               // Insert the data
               if (id == SmsProviderRequestEnum.SESSION_ID.getId()) {
   ...
   /**
    * This method unpacks a byte array containing a SmsProviderRequest object.
    *
    * @return The byte array as a SmsProviderRequest message object.
    */
   public SmsProviderRequest unpack(byte[] ba) {
      ...
      for (GreenballMessagesDirectoryEntry directoryEntry : theDirectory) {
         ...
         if (id == SmsProviderRequestEnum.SESSION_ID.getId()) {
            smsProviderRequest.setSessionId(GreenballMessagesUtility.getString(ba, offset, length));
         }
      }
   ...
   /**
    * This method packs a SmsProviderRequest object into a String. If a required attribute is not
    * provided an exception will be thrown.
    *
    * @return A String which represents a SmsProviderRequest message object.
    * @throws MissingAttributeException
    */
   public String packStr() throws MissingAttributeException {
       ...
       */
      InitializeSmsProviderRequest initializeSmsProviderRequest = new InitializeSmsProviderRequest(this);
      Element element = initializeSmsProviderRequest.initialize();
    ...
    */
   public SmsProviderRequest unpack(String buffer) {
      final String METHOD = "unpack()";
      log4j.debug("Entered " + METHOD);
      ...
      /*
       * Instantiate the SmsProviderRequest object.
       */
       InstantiateSmsProviderRequest instantiateSmsProviderRequest = new InstantiateSmsProviderRequest(element);
       SmsProviderRequest smsProviderRequest = instantiateSmsProviderRequest.instantiate();

      /*
       * Verify that a value was provided for all mandatory attributes.
       */
      String result = instantiateSmsProviderRequest.validate();
      if (result != null) {
    ...
    */
   private int getSmsProviderRequestDirectorySize() {
      int directorySize = 0;
   ...
   /**
    * This method gets the size of the SmsProviderRequest object.
    * @return The size in bytes.
    */
   public int getSmsProviderRequestSize() {
      int size = 0;
   ...
   public boolean equals(Object obj) {
      ...
      SmsProviderRequest smsProviderRequest = (SmsProviderRequest) obj;
      if (!smsProviderRequest.getSessionId().equals(this.getSessionId())) {
         return false;
      }
