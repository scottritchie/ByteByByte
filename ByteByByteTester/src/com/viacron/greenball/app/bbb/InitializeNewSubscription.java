...
/**
 * The InitializeNewSubscription class initializes the element object with the NewSubscription object.
 *
 */
public class InitializeNewSubscription {
   private static Logger log4j = Logger.getLogger(InitializeNewSubscription.class);
    ...
    * The InitializeNewSubscription class initializes the element object with the NewSubscription object.
    *
    * @param newSubscription The object which will be used to initialize the element object.
    */
   public InitializeNewSubscription(NewSubscription newSubscription) {
      this.newSubscription = newSubscription;

      this.element = new Element("NewSubscription");
   }
   ...
   public Element initialize() throws MissingAttributeException {
      ...
      log4j.debug("element.getName(): NewSubscription");

      if (newSubscription.getOrganization() != null) {
         AbstractAttribute abstractAttribute = initializeOrganization(newSubscription.getOrganization());
      ...
      if (newSubscription.getSessionId() != null) {
         AbstractAttribute abstractAttribute = initializeSessionId(newSubscription.getSessionId());
         element.getAbstractAttributes().add(abstractAttribute);
      }
      else {
         // Missing required attribute, generate an error message and throw an exception
