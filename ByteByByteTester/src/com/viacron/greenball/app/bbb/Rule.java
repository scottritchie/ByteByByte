...
public class Rule {
   ...
   private enum RuleEnum {
      ...
      // sizeInBits: 32
      EXPECTED_NODE_STATE(20, 69, 0, false),

      // sizeInBits: 8
      ENABLED(21, 101, 0, false),

      // sizeInBits: 8
      EMAIL_NOTIFICATION(22, 109, 0, false),

      // sizeInBits: 8
      SMS_NOTIFICATION(23, 117, 0, false),
      ...
      // sizeInBits: 8
      SAVE_WHEN_EXPIRED(25, 125, 0, false),

      // sizeInBits: 8
      EXPIRED(26, 133, 0, false),

      // sizeInBits: 3
      // sizeInBytes: 1
      STATE(27, 141, 0, false);
   ...
   public byte[] pack() throws MissingAttributeException {
      ...
      bb = GreenballMessagesUtility.insertBitfield(bb, offset * 8, 3, getState().ordinal());
      // bb = GreenballMessagesUtility.insertInteger(bb, offset, this.getState().getId());
      offset += 1;
      ...
      RuleEnum.TEST_PATH_I_DS.setLength(this.getTestPathIDs().size());
      RuleEnum.SCOPE_I_DS.setLength(this.getScopeIDs().size());
      RuleEnum.TEST_I_DS.setLength(this.getTestIDs().size());
      RuleEnum.ORG_ID.setLength(this.getOrgId().length());
      RuleEnum.NAME.setLength(this.getName().length());
      RuleEnum.ID.setLength(this.getId().length());
      RuleEnum.TEST_PATH_LABELS.setLength(this.getTestPathLabels().length());
      RuleEnum.TEST_PATH_IDS.setLength(this.getTestPathIds().length());
      RuleEnum.DEADLINE_PATH_LABELS.setLength(this.getDeadlinePathLabels().length());
      RuleEnum.DEADLINE_PATH_IDS.setLength(this.getDeadlinePathIds().length());
      RuleEnum.SCOPE_PATH_LABELS.setLength(this.getScopePathLabels().length());
      RuleEnum.SCOPE_PATH_IDS.setLength(this.getScopePathIds().length());
      RuleEnum.WARNING.setLength(this.getWarning().length());
      RuleEnum.TIME_PERIOD.setLength(this.getTimePeriod().length());
      RuleEnum.EMAIL_ADDRESS.setLength(this.getEmailAddress().length());
      RuleEnum.SMS.setLength(this.getSms().length());
      RuleEnum.DEADLINE_ID.setLength(this.getDeadlineId().length());
   ...
   public Rule unpack(byte[] ba) {
      ...
      rule.setState(RuleState.toEnum(GreenballMessagesUtility.getInteger(ba, RuleEnum.STATE.getOffsetInBits())));
